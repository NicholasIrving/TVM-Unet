...
class nas(nn.Module):
    def __init__(self, task, tune_option, device):
        ...
        p = PolicyGradient(task, tune_option, device)
        p.solve_environment()


class Agent(nn.Module):
    def __init__(self, input_size, hidden_size=64, num_steps=8, device=''):
...
class PolicyGradient:
    def __init__(self, task, tune_option, device):
        ...
        self.NUM_STEPS = len(task.config_space.space_map)
    ...
    def play_episode(self):
        ...
        """
        # Init state
        init_state = torch.zeros(len(self.task.config_space.space_map))
        ...
        cc = softmax(episode_logits, dim=1)
        aa = log_softmax(episode_logits, dim=1)
        bb = mask.float() * log_softmax(episode_logits, dim=1)
